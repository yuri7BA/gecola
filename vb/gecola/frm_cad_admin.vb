...
Public Class frm_cad_admin
    ...
    Public foto_perfil As String = "administrador.png"
    Public Const PERFIL As Char = "a"

    '-------------------'
    ' FORMULÁRIO ABERTO '
    '-------------------'
    Private Sub abrir_frm_cad_admin() Handles MyBase.Load
        sexo = ""
        gbDPOk = False
        gbAutOk = False
        gbEndOk = False
    ...
    Private Sub carregar_foto() Handles pb_foto.Click, pb_foto.DoubleClick
        Try
            With janela_selecionar_foto
                ...
                .InitialDirectory = Application.StartupPath & "\recursos\fotos\administradores"
    ...
    Private Sub verificar_groupboxes() Handles btn_salvar_topo.Click, btn_salvar_base.Click
        ...
        If gbDPOk And gbAutOk And gbEndOk And gbConOk Then 'todas groupboxes foram devidamente preenchidas (esta parte do código NÃO troca a ativação do perfil)
            qry = "SELECT * FROM tb_usuarios WHERE cpf='" & cpf & "' and perfil='" & PERFIL & "'" 'verifica se o CPF informado já está cadastrado como administrador
            rs = db.Execute(qry)
            If Not rs.EOF Then 'o CPF realmente já existe no banco como administrador
                resposta = MsgBox("Esse CPF já está cadastrado como administrador. Você está tentando alterar os dados cadastrais?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "ERRO")
                If resposta = vbYes Then
                    ...
                    If Not rs.EOF Then
                        ...
                        txt_usuario.Select()
                        txt_usuario.SelectAll()
                    Else 'atualizar o CPF que já está cadastrado como administrador
                ...
                Else
                    ...
                    txt_cpf.SelectAll()
                End If
            Else 'o CPF informado ainda não existe (como administrador) no banco
    ...
    ' FORMULÁRIO FECHADO '
    '--------------------'
    Private Sub fechar_frm_cad_admin() Handles MyBase.FormClosed
