...
Imports Microsoft
Public Class frm_cad_funcionario
    ...
    Public foto_perfil As String = "funcionario.png"
    Public Const PERFIL As Char = "f"

    '-------------------'
    ' FORMULÁRIO ABERTO '
    '-------------------'
    Private Sub abrir_frm_cad_funcionario() Handles MyBase.Load
        sexo = ""
        gbDPOk = False
        gbAutOk = False
        gbEndOk = False
    ...
    Private Sub carregar_foto() Handles pb_foto.Click, pb_foto.DoubleClick
        Try
            With janela_selecionar_foto
                ...
                .InitialDirectory = Application.StartupPath & "\recursos\fotos\funcionarios"
    ...
    Private Sub verificar_groupboxes() Handles btn_salvar_topo.Click, btn_salvar_base.Click
        ...
        If gbDPOk And gbAutOk And gbEndOk And gbConOk Then 'todas groupboxes foram devidamente preenchidas (esta parte do código NÃO troca a ativação do perfil)
            qry = "SELECT * FROM tb_usuarios WHERE cpf='" & cpf & "' and perfil='" & PERFIL & "'" 'verifica se o CPF informado já está cadastrado como funcionário
            rs = db.Execute(qry)
            If Not rs.EOF Then 'o CPF realmente já existe no banco como funcionário
                resposta = MsgBox("Esse CPF já está cadastrado como funcionário. Você está tentando alterar os dados cadastrais?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "ERRO")
                If resposta = vbYes Then
                    ...
                    If Not rs.EOF Then
                        ...
                        txt_usuario.Select()
                        txt_usuario.SelectAll()
                    Else 'atualizar o CPF que já está cadastrado como funcionário
                ...
                Else
                    ...
                    txt_cpf.SelectAll()
                End If
            Else 'o CPF informado ainda não existe (como funcionário) no banco
    ...
    ' FORMULÁRIO FECHADO '
    '--------------------'
    Private Sub fechar_frm_cad_funcionario() Handles MyBase.FormClosed
